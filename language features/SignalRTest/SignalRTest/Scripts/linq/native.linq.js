function linq(massiv) {
    ...
    var linqArray = massiv === undefined || massiv === null ? [] : convertToArray(massiv);

    //if (!isArray(linqArray)) {
    ...
    linqArray.infoColumn = function (index) {
        var context = this;

        var name = getColumnName(context, index);

        if (name != null)
            return { AllIsEmpty: context.all("x=>x." + name + '==""'), AllIsNull: context.all("x=>x." + name + '== null') };
